...
/*
  ...
  Widget downloadBar() {
    ...
    if (ref.read(epubProvider).status == MyEpubStatus.downloadable) {
      if (all == 1) {
        // 全1話
        req1 = all;
        req2 = 0;
    ...
    } else if (ref.read(epubProvider).status == MyEpubStatus.succeeded) {
      // 成功
      label2 = '${l10n('download_complete')} ${done} / ${all}';
      isClose = true;
    } else if (ref.read(epubProvider).status == MyEpubStatus.same) {
      // 最新
      label2 = '${l10n('already_downloaded')} ${already} / ${all}';
      isClose = true;
    } else if (ref.read(epubProvider).status == MyEpubStatus.failed) {
      // 失敗
      label2 = '${l10n('download_failed')}';
      isClose = true;
    } else if (ref.read(epubProvider).status == MyEpubStatus.downloading) {
      // ダウンロード中
      label1 = 'Downloading';
      if (done > 0) {
        label1 += ' ${done} / ${all}';
